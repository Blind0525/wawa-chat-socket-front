<template>
  ...
  <div v-else class="cs-overlay">
    <div class="cs-header">
      ...
      <span class="cs-title">{{ customerName || '在线客服' }}</span>
  ...
  <div v-if="callState !== 'idle'" class="cs-call-overlay">
    ...
    <template v-if="callState === 'ringing'">
      ...
      <div class="cs-call-title">顾客来电</div>
...
import { chatGetMessagesApi, chatUploadFileApi, chatMarkReadApi } from '@/utils/http/ChatApi'
...
let connectCheckTimer = null  // 发起/接听后 60 秒未接通判定失败
...
const customerName = ref('')      // 顾客名称(会话列表传入,头部显示)
...
/** 客服进入会话:连接 ws + 加载历史 + 标记已读 */
async function agentInit() {
  pageState.value = 'loading'
  loadingText.value = '正在加载会话...'
  try {
    connectWs()
...
function connectWs() {
  ws = new ChatSocket({
    ...
    onMsg: (data) => {
      // 只处理当前会话的消息
      if (data.sessionId && Number(data.sessionId) !== Number(sessionId)) return
...
async function sendMsg() {
  ...
  if (!text || !wsConnected.value || !peerUserId) return
  ...
  ws.send({ type: 'text', to: peerUserId, sessionId, content: text, localId })
}
...
function createPeer() {
  ...
  pc.onicecandidate = (e) => {
    if (e.candidate && peerUserId) {
      ws.send({ type: 'call', action: 'candidate', to: peerUserId, sessionId, candidate: e.candidate })
    }
  }
...
async function startCall(type) {
  if (!wsConnected.value || !peerUserId || callState.value !== 'idle') return
  callType.value = type
  ...
  try {
    ...
    ws.send({ type: 'call', action: 'invite', to: peerUserId, sessionId, callType: type, sdp: pc.localDescription })
    startConnectCheck()
  } catch (e) {
...
async function acceptCall() {
  ...
  try {
    ...
    ws.send({ type: 'call', action: 'accept', to: pendingOffer.from || peerUserId, sessionId, sdp: pc.localDescription })
  } catch (e) {
...
/** 拒绝来电 */
function rejectCall() {
  ws.send({ type: 'call', action: 'reject', to: pendingOffer.from || peerUserId, sessionId })
  endCall()
}

/** 挂断/取消 */
function hangUpCall() {
  ws.send({ type: 'call', action: 'hangup', to: peerUserId, sessionId, duration: callTimer.value })
  endCall()
}
